...
def question10():
    ...
    # type: explanatory string
    answers["(e) A and B conditionally independent given class +, explain"] =  "In essence a, and b are independent given the class+ given. This covers mainly on the fact that we need independence of conditionality within the time NO conditionality constraints exists in Naïve Bayes classifier in the framework of conditional independence constraints for A and B should be considered independent under final class if P(A&B/class)=P(A/class) * P(B/class)."
